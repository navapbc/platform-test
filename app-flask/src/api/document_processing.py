...
class DocumentProcessingRequestSchema(request_schema.OrderedSchema):
    trace_id: str = fields.Str(required=False, metadata={"description": "Optional trace ID for tracking"})
    file_key: str = fields.Str(required=True, metadata={"description": "S3 key of the file to process"})
...
document_processing_blueprint = APIBlueprint("document_processing", __name__, tag="Document Processing")
...
def test_document_data_extraction_processing(json_data: Dict[str, Any]) -> Tuple[response.ApiResponse, int]:
    ...
    if not input_bucket or not project_arn:
        logger.error(f"Missing DDE configuration: input_bucket={input_bucket}, project_arn={project_arn}")
        return response.ApiResponse(
            message="DDE configuration not available",
            errors=[response.ValidationErrorDetail(type="configuration", message="Missing DDE environment variables")]
        ), BadRequest.code
    ...
    try:
        ...
        bedrock_request = {
            ...
            "outputConfiguration": {
                "s3Uri": f"s3://{output_bucket}/{file_key}"
            }
        }
        ...
        logger.info(f"DDE invocation successful for file: {file_key}, invocation_arn: {bedrock_response.get('invocationArn')}")
