...
The infrastructure uses AWS GuardDuty for threat detection and security monitoring across the AWS account. GuardDuty is enabled by default to continuously analyzes the following data sources:
...
The infratructure leverages GuardDuty's malware detection feature to continuously scans files uploaded to S3 buckets for malicious content. This is enabled by default.  When malware is detected:
...
1. Edit Terraform configuration file for the account layer,`infra/project-config/threat_detection.tf`
