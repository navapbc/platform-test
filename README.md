...
| app-docuai <br /> ([source](https://github.com/navapbc/strata-template-documentai-api))      | https://app-docuai.platform-test-dev.navateam.com |                                    |
...
You can view API docs at `/docs` on the above URLs, in [dev for
example](https://app-docuai.platform-test-dev.navateam.com/docs).

To retrieve the API key for use in the dev environment:

```bash
aws ssm get-parameter --name "/app-docuai-dev/api-auth-insecure-shared-key" --with-decryption --query "Parameter.Value" --output text
```
